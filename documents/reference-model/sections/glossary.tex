...
Many of the definitions are amended with notes and references to RAMI4.0 \cite{adolphs2016reference} and SOA-RM \cite{mackenzie2006reference}, which are always listed after the definition they amend.
Regular notes are numbered, while those making a comment on a definition in RAMI4.0 or SOA-RM are introduced with the two abbreviations just mentioned.
...
A pool of \GlossaryHyperRef{system}{systems} that can execute tasks provided by external \GlossaryHyperRef{user}{users} or other \GlossaryHyperRef{entity}{entities} satisfying its \GlossaryHyperRef{policy}{policies}.

\GlossaryEntry{cloud-local}{Cloud, Local}
A \GlossaryHyperRef{cloud}{cloud} whose \GlossaryHyperRef{system}{systems} may act on physical \GlossaryHyperRef{artifact}{artifacts}, meaning it is practically bound to a physical location.

\GlossaryEntry{cloud-virtual}{Cloud, Virtual}
A \GlossaryHyperRef{cloud}{cloud} whose \GlossaryHyperRef{system}{systems} only act on virtual \GlossaryHyperRef{artifact}{artifacts}, meaning it is not strictly bound to a physical location.
...
A \GlossaryHyperRef{description}{description} of a \GlossaryHyperRef{design}{design}, especially in terms of how a particular engineering task can be accomplished with respect to that design.

	\GlossaryNote{Note 1}{
		Design descriptions, which can be seen as targeted summaries of design documents, could be written to help engineers integrate, configure, migrate, decommission or install certain systems or other entities, among other possible examples.
	}
...
A set of \GlossaryHyperRef{function}{functions} that can be \GlossaryHyperRef{provider-service}{provided} via a \GlossaryHyperRef{interface-service}{service interface}.
	...
	\GlossaryNote{Note 1}{
		...
		This introduces the need to be able to determine how to forward \GlossaryHyperRef{message}{messages} received by the service to the correct functions.
