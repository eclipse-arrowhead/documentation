...
\item \textit{is}, which means that the source entity \textit{extends} and belongs to a set named after the target entity;
\item \textit{has}, which means that the target entity cannot exist without the source entity;
