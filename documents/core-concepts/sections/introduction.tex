...
In a graph diagram, a box with a solid border and a name inside it denotes a named model \GlossaryHyperRef{entity}{entity} and may represent any \GlossaryHyperRef{artifact}{artifact} or \GlossaryHyperRef{stakeholder}{stakeholder}.
...
\begin{enumerate}
\item \textit{conforms to}, which implies that the target entity \textit{is} a \GlossaryHyperRef{constraint}{constraint} satisfied by the source entity;
\item \textit{is} (or \textit{extends}), which means that the source entity \textit{conforms to} the target entity, inherits all of its relationships and implicit \GlossaryHyperRef{attribute}{attributes}, as well as belongs to a set named after it;
