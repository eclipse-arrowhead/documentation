...
Whenever any of these stages receives a message, it will either \textit{send} it leftwards or \GlossaryHyperRef{routing-message}{\textit{route}} it rightwards, depending on whichever direction brings the message closest to the \GlossaryHyperRef{operation-service}{service operation} it targets.
...
At that and every other stage on the journey leftwards, the message may be tested against the \GlossaryHyperRef{state-protocol}{protocol state} and/or policies expected to be enforced by the corresponding interfaces of the target.
...
For example, a device interface being a \GlossaryHyperRef{interface-network}{network interface} supports making this jump via a \GlossaryHyperRef{network}{network}.
...
\textit{Message types} determine what \GlossaryHyperRef{data}{data} valid messages must and may contain, while each \textit{state type} dictates when certain messages are valid in relation to a certain \GlossaryHyperRef{state-protocol}{state}.
As shown in Figure \ref{fig:protocol}, a protocol may also be defined as an \GlossaryHyperRef{protocol-extensible}{extension} of another protocol, as well as be constrained by certain \GlossaryHyperRef{profile-protocol}{profiles} and \GlossaryHyperRef{encoding}{encodings}, described in Sections \ref{sec:concepts:profile} and \ref{sec:concepts:encoding}.
...
Note that protocols should only be concerned with the \textit{interpretation} of messages, not with whether they are \GlossaryHyperRef{message-permitted}{\textit{permitted}} or not.
...
A \GlossaryHyperRef{policy}{policy} is an \GlossaryHyperRef{identifiable}{identifiable} set of \GlossaryHyperRef{constraint}{constraints}, useful for determining if given \GlossaryHyperRef{message}{messages} are permitted or not, as depicted in Figure \ref{fig:policy}.
...
Every policy may, when relevant, be regarded as a \GlossaryHyperRef{function-predicate}{predicate function} useful for testing if given message is \GlossaryHyperRef{message-permitted}{permitted} or \GlossaryHyperRef{message-forbidden}{forbidden}, with respect to any kind of information.
...
If a \GlossaryHyperRef{message}{message} fails to satisfy one or more of the policies of an interface, the message sender should be notified about the violated policies.

While \GlossaryHyperRef{protocol}{protocols} help determine if a given message can be interpreted correctly, \textit{policies} are meant to determine if the activity described by that message would occur under desirable conditions.
...
A \textit{profile} is a set of \GlossaryHyperRef{constraint}{constraints} added to every message of a protocol.
