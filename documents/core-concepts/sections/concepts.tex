...
As depicted in Figure \ref{fig:service}, services have three significant types of \GlossaryHyperRef{component-software}{software components}.
These are (1) \GlossaryHyperRef{policy}{policies}, (2) \GlossaryHyperRef{operation-service}{operations} and (3) \GlossaryHyperRef{interface-service}{service interfaces}.
...
\begin{figure}[ht!]
  ...
  \caption{
    ...
    While not made explicit by this figure, operations are facilitated by the \GlossaryHyperRef{state-software}{states} and \GlossaryHyperRef{procedure-software}{procedures} of the systems that provide them.
  }
  \label{fig:service}
\end{figure}

Services receive messages via their interfaces, which \GlossaryHyperRef{routing-message}{route} them to the operations they target.
See Section \ref{sec:concepts:interface} for more details about interfaces.
An operation receiving a message must guarantee that it adheres to its \GlossaryHyperRef{protocol-operation}{protocol} and satisfies all of its \GlossaryHyperRef{policy-operation}{policies}, after which it must concretely handle the request described in the message.
