...
As shown in Figure \ref{fig:protocol}, a protocol may also be defined as an \GlossaryHyperRef{protocol-extensible}{extension} of another protocol, as well as conform to certain \GlossaryHyperRef{profile-protocol}{profiles} and \GlossaryHyperRef{encoding}{encodings}, described in Sections \ref{sec:concepts:profile} and \ref{sec:concepts:encoding}.
...
A \textit{profile} is a set of \GlossaryHyperRef{constraint-protocol}{constraints} added to every \GlossaryHyperRef{message}{message} of the \GlossaryHyperRef{protocol}{protocols} that elect to conform to it.
As depicted in Figure \ref{fig:profile}, the constraints are formulated in terms of a protocol that that is extended by the protocols that elect to conform to it.

\begin{figure}[ht!]
  \centering
  \includegraphics[scale=0.9]{figures/profile}
  \caption{
    The profile as a set of protocol constraints.
  }
  \label{fig:profile}
\end{figure}
...
If that custom protocol elects to conform to a certain profile, the constraints of that profile must be formulated in terms of HTTP without the extension or any other protocol HTTP extends, such as TCP/IP \cite{postel1981transmission} or IP \cite{deering2017internet}.
...
This enables a profile to be concerned only with \GlossaryHyperRef{message-type}{message types}, perhaps useful as payloads for protocols electing to conform to it.

A profiles could specify how authorization tokens are to be included in messages, demand that a certain message semantics, such as SenML \cite{rfc8428}, be observed, and so on.
Note that adding a profile to a protocol not already conforming to it produces a new protocol, as implied in Figure \ref{fig:protocol}.
