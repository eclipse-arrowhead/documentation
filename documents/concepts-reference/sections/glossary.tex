...
A \GlossaryHyperRef{stakeholder}{stakeholder} who seeks to improve upon or extend the \GlossaryHyperRef{framework-arrowhead}{Arrowhead framework} itself, by, for example, writing core documentation or producing architectural \GlossaryHyperRef{description}{descriptions}.
See Section \ref{sec:concepts:stakeholder}.

\GlossaryEntry{architecture}{Architecture}
A \GlossaryHyperRef{model}{model} of a \GlossaryHyperRef{system-of-systems}{system-of-systems} defined in terms of a certain (1) goals, ambitions or other principles; (2) an environment, either abstract or concrete; (3) as well as significant life-cycle events, such as construction, maintenance or decommissioning.
...
See Section \ref{sec:concepts:local-cloud}.
...
An \GlossaryHyperRef{artifact}{artifact} that can be part of another artifact and contribute to it facilitating its \GlossaryHyperRef{capability}{capabilities}.
	...
	\GlossaryNote{Note 1}{
		The term ``component'' should never be used to refer to a system being a constituent of a \GlossaryHyperRef{system-of-systems}{system-of-systems}.
...
\GlossaryEntry{constraint-encoding}{Constraint, Encoding}
A \GlossaryHyperRef{constraint}{constraint} imposed on an \GlossaryHyperRef{encoding}{encoding}.
See Section \ref{sec:concepts:profile}.

	\GlossaryNote{Note 1}{
		An encoding constraint could require that an optional \GlossaryHyperRef{data}{data} field be present or omitted, require that the values of certain fields satisfy a certain \GlossaryHyperRef{function-predicate}{predicate function}, and so on.
	}
...
\GlossaryEntry{constraint-protocol}{Constraint, Protocol}
A \GlossaryHyperRef{constraint}{constraint} imposed on a \GlossaryHyperRef{protocol}{protocol}.
See Section \ref{sec:concepts:profile}.

\GlossaryEntry{consumer-service}{Consumer, Service}
A \GlossaryHyperRef{system}{system} currently \GlossaryHyperRef{consumption-service}{consuming} a \GlossaryHyperRef{service}{service} by sending a \GlossaryHyperRef{message}{message} to one of its \GlossaryHyperRef{operation-service}{operations}.
...
The act of consuming a \GlossaryHyperRef{service}{service} by sending a \GlossaryHyperRef{message}{message} to one of its \GlossaryHyperRef{operation-service}{operations}.
...
A conceptual mathematical construct that transforms given input values into output values.
	...
	\GlossaryNote{Note 1}{
		Most functions can be \GlossaryHyperRef{implementation}{implemented} as \GlossaryHyperRef{implementation-software}{software}.
	}
...
Every message must \GlossaryHyperRef{identification}{identify} a target \GlossaryHyperRef{operation-service}{service operation} and may contain \GlossaryHyperRef{metadata}{metadata} and a payload.

	\GlossaryNote{Note 1}{
		The metadata of a message represent the details it contains about its transmission and interpretation.
		Metadata are always concerned with the \GlossaryHyperRef{protocol}{protocol} of the message and with satisfying the \GlossaryHyperRef{policy}{policies} of the interfaces that must be passed on its journey to its target operation.
		Metadata may be added, modified and/or used by \GlossaryHyperRef{interface}{interfaces} when messages pass through them.
	}

	\GlossaryNote{Note 2}{
		A payload is an input to a service operation.
		Operations are not required to expect input data, which is why having a message payload is optional.
	}

\GlossaryEntry{message-error}{Message, Error}
A \GlossaryHyperRef{message}{message} indicating why the request expressed by some other message could not be executed.

	\GlossaryNote{Note 1}{
		We expect the receiver of most error messages to be the respective senders of the messages that could not be executed.
	}
...
An activity a \GlossaryHyperRef{service}{service} can perform if receiving a \GlossaryHyperRef{message-valid}{valid} and \GlossaryHyperRef{message-permitted}{permitted} \GlossaryHyperRef{message} targeting that operation.
See Section \ref{sec:concepts:service}.

\GlossaryEntry{operation-service}{Operation, Service}
Prefer \GlossaryNameRef{operation}.
...
Prefer \GlossaryNameRef{routing}{routing}.
...
The property of being virtual, as opposed to being physical.
...
See Section \ref{sec:concepts:solc}.

\GlossaryEntry{system-of-systems}{System, of-Systems (SoS)}
A set of \GlossaryHyperRef{system}{systems} that \GlossaryHyperRef{consumer-service}{consume} each other's \GlossaryHyperRef{service}{services} in order to facilitate a \GlossaryHyperRef{capability-system}{capability} none of the constituent systems could \GlossaryHyperRef{provision-service}{provide} on its own.
